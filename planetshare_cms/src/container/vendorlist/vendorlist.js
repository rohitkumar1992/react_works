...
import {VENDOR_LIST,VIDEO_CHANGESTATUS} from '../../url.js';
...
this.handlePageChange('1');
   ...
   var email=localStorage.getItem('email');
     axios.post(`${VENDOR_LIST}?page=${pageNumber}`, {
       email:email,
  ...
  render()
  {
    ...
    return (
      <tr>
      <td ><Link to={`/vendor/details/${result.user_id}`}><a href="#" class="link_col">{result.accountId}</a></Link></td>
            <td class="ser_width">{result.name}</td>
      <td class="ser_width">{result.companyName}</td>
  ...
  {
  return (<Loader content="Vendor List"/>);
  }
  else {
    return (
      <div >
