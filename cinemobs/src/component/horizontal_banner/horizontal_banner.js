                  ...
                  <div class={`slider_box ${props.result.cat_type!="cat"?'sub_cat':''}`} >
                      ...
                      <OwlCarousel
                        ...
                        {props.result.search_tag.map((res,key)=>{return(<div class="item">
                                {(props.uniqId!="season_list" && props.result.cat_type=="cat") && <Link to={`/video/${(res.name.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g,'')).toLowerCase()}/${res.entryid}`} class={`box`}>
                                  <div class="img">
                                  ...
                                  <div class="det">{res.description.substr(0,100)}</div>
                                  </div>
                              </Link>}

                              {((props.result.cat_type=="sh" || props.result.cat_type=="ser" || props.result.cat_type=="sea") && props.uniqId!="season_list") && <Link to={(props.result.cat_type=="sh" && res.sub_count==0)?`/video/${(res.name.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g,'_')).toLowerCase()}/${res.id}`:res.sub_count==null?`/cat/${props.route_name}/${(res.name.replace(/\s/g,'_')).toLowerCase()}/${res.id}`:`/season/${(res.name.replace(/\s/g,'')).toLowerCase()}/${res.id}`}>
                                <div class="img">
                            ...
                            {(props.uniqId=="season_list") && <Link to={`/video/${(res.name.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g,'')).toLowerCase()}/${res.entryid}`} class={`box`}>
                              <div class="img">
                                  <img  class="load_place_horiz" src={res.thumburl.h_thumburl} alt="" style={{maxHeight:'160px'}}/>
                                  <p class="duration"><Duration sec1={res.duration}/></p>
                              </div>
