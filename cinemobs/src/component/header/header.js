...
class Header extends React.Component
{
  ...
  logOut=()=>{
    ...
    //  {
      ...
      //}
      // if(response.data.ErrorCode=='E000444')
      // {}
    }).catch((error)=>{
      console.log(error);
    })
  ...
  render()
  {
    ...
    const MenuItems=menuItems.map((result,key)=>{
      if(result.view_type!==0)
      {
        ...
        return(<li class={`nav-item ${(route_name.length==0 && result.catName=='HOME')?'active':result.catName.replace(/\s/g,'').toLowerCase()==route_name?'active':''}`}>
            <Link to={`/${(result.catName.replace(/\s/g,'')).toLowerCase()}/kog_livetv/${result.catID}`}  class="nav-link" >{result.catName.charAt(0).toUpperCase() + result.catName.slice(1).toLowerCase()} </Link>
        </li>);
      }
    })
                      ...
                      <div class="menu_div">
                          <ul class="navbar-nav f collapse navbar-collapse" id="collapsibleNavbar">
                              ...
                              <ul class="navbar-nav float-right">
                               <Search {...this.props} />
                               {localStorage.getItem('userid')==0 && <li class="nav-item">
                               ...
                               {localStorage.getItem('userid')!=0 && <li class="nav-item dropdown">
                                   ...
                                   <div class="dropdown-menu">
                                      {this.props.leftMenu.length>0 && this.props.leftMenu.map((result,key)=>{
                                       ...
                                       <a class="dropdown-item" href="javascript:;" onClick={()=>{this.props.history.push(`/cinemobs_user/profile/user${USERID}`)}}><i class="fa fa-user" aria-hidden="true"></i> Profile</a>
                                      <a class="dropdown-item" href="javascript:;" onClick={()=>{this.props.history.push('/collection')}}><i class="fa fa-hand-lizard-o" aria-hidden="true"></i> Your Collection</a>
