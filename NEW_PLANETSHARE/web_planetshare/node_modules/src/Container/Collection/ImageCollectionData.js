...
class ImageCollectionData extends React.Component{
    ...
    if(!not_found)
    {
            const Gallery=(collection_data.length>0 && collection_data.map((result,key)=>{
              return(<div class="item" key={key}>
                        <Link to={`/web/image/${(result.image_item_info.title.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g,'')).toLowerCase()}/${result.image_item_info.id}`} class="box">
                                         <div class="img">
                                              <img class="load_place_vert" src={result.image_item_info.large_thumb} alt=""/>
                                          </div>
                                          <div class="desc">
                                              <span class="title">{result.image_item_info.title}</span>
                                              <div class="det" dangerouslySetInnerHTML={this.createMarkup(result.image_item_info.short_desc)} ></div>
                                          </div>
                                      </Link>
                                      <div class="sv_crt">
                                          <button type="button" class="crt" onClick={()=>this.addToDownload(result.image_item_info.id)}>
...
export default Authentication(ImageCollectionData);
