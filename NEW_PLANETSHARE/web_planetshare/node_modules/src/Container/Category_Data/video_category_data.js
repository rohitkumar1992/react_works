...
import {GETVIDEOCATEGORYDATA,USERID,TAG,GETVIDEOFILTERCONTENT,DOWNLOADS} from '../../url';
...
class ImageCategoryData extends React.Component{
  ...
  {
    super(props);
  this.state={isLoading:false,loading_msg:'Please Wait',cat_id:'',cat_name:'',not_found:false,category_data:[],
  ...
  getData=(page,limit,keyword)=>{
    axios.post(GETVIDEOCATEGORYDATA,{
      'user_id':USERID,
      'tag':TAG,
      'limit':limit,
  ...
  getFilterContent=()=>{
    axios.post(GETVIDEOFILTERCONTENT,{
      'user_id':USERID,
      'tag':TAG,
      'category_id':this.state.cat_id,
    }).then((res)=>{
...
const {isLoading,not_found,category_data,cat_name,total_pages,total_result,filter_artist_data,loading_msg,keywords_final}=this.state;
    if(!not_found)
    {
      const Video_Listing=(category_data.length>0 && category_data.map((res,key)=>{
              ...
              <div class="box">
                <Link to={`/web/video/${(res.title.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g,'')).toLowerCase()}/${res.id}`}>
                  <p id={`load_${res.id}`} style={{display:'none'}} >Loading...</p>
                    <video ref={c => this._nodes.set(res.id, c)}  src={res.demo_url} type="video/mp4" onMouseEnter={(e)=>this.playVideo(e,res.id)} onMouseLeave={(e)=>this.pauseVideo(e,res.id)} style={{width:"250px"}}
                    poster={res.large_thumb}
                    ></video>
                      ...
                      <div class="q_t">
                          ...
                          <span class="ttle">{res.title}</span>
                      </div>
                  </Link>
                  <div class="sv_crt">
      ...
      // const Keywords=(category_data.length>0 && category_data.map((res,key)=>{
      //   return(<div key={key}>{res.keywords.split(',').slice(0,3).map((result,key1)=>{
      //     return(<a href="javascript:;" class="btn btn_catlist">{result}</a>)
      //   })}</div>)
      // }));
          ...
          <section class="category_page top_div">
              ...
              <div class="cat_filter_area">
                                      ...
                                      <div class="card-body">
                                          <div class="row">
                                              <div class="col col-sm-6">
                                                  <button type="button" class="btn btn-success">Popular</button>
                                              </div>
                                              <div class="col col-sm-6">
                  ...
                  <div class="cat_content_area">
                      <div class="top_info">
                          <h2>{cat_name} royalty-free stock videos</h2>
                          <p>
                              {total_result} {cat_name} royalty-free stock videos <a href="javascript:;">See {cat_name} stock videos</a>
                          </p>
