...
import {TAGGING_ORDERS} from '../../../../url';
...
class DubbingOrders extends Component {
  ...
  action = (tagging_id,payment_id,status,title)=>{
    if(payment_id==null)
    {
      return(<div><Link to={`/dashboard/buyer/service/tagging/preview/${tagging_id}`}><b style={{width:'2em',height:'2em',cursor:'pointer',color:'green'}} >Make Payment</b></Link></div>);
    }
    else
    {
      return ( status=='2'?<div>
                 <Link to={`/dashboard/buyer/service/tagging/myorders/preview/${tagging_id}`}>
  ...
  render()
  {
    const table=(this.state.myorders==''?<tr class="table_content"><td colspan="4" class="width_set">No Records Found</td></tr>:this.state.myorders.map((result,key)=>{
      return (<tr class="table_content">
                {/*<td><center><img src={`http://192.168.24.132/${result.thumbnail}`} class="table_img"/></center></td>*/}
                <td class="width_set">{result.autotag_id}</td>
    ...
    return(
      <div class="tagging_myorder" style={{marginTop:'20px'}}>
      <div class="row" >
