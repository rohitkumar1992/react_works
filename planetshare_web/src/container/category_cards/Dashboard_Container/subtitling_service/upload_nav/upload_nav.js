...
class UploadNav extends Component {
  ...
  render()
  {
    ...
    return <VideoList video_id={result.video_id} tag="subtitle_service" img_url={`${result.thumbnail}`} host_url={result.host_url} title={result.title} description={result.description} video_tag={result.Premium=='0'?'Free':'Premium'} price={result.Premium=='0'?'0':result.Price}/>
  }));
    return(
